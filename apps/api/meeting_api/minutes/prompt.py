...
MINUTES_PROMPT_INSTRUCTIONS = (
    ...
    "超过 90 分钟的长会可放宽到 5～8 个——按实际信息量伸缩，"
    "不要让会议后半段的讨论被挤进一个大议题里过度压缩。"
    ...
    "人名、机构、渠道、账号名等具体线索同样保留，不要泛化成「某资源」"
    "「相关渠道」。\n"
    "- 同一价格、日期或方案在会中被多次修改时，以会议最终认定的口径写入要点，"
    "不要把演进过程抹平成一个区间；早期版本仅在对比有意义时用括号简要带过"
    "（如「面试单节 2800 元（此前拟 2600）」）。但被提出后仍悬而未决的备选方案"
    "及其具体数字（如更高档定价选项、可能的提价空间），不要当作旧版本丢弃"
    "——写进对应要点或「待决问题与风险」。\n"
    ...
    "\n"
    "七、`## 待决问题与风险`：罗列会上明确提出、但散会时仍没有结论、负责人或"
    "时间点的问题与风险，每条一行、写清悬在哪里（如「付款主体是学校还是学生"
    "未定」「导师质控流程尚无方案」）；只收录逐字稿里真实出现过的疑问或风险，"
    "不要自行推演，没有就写「无」。\n"
    "\n"
    "硬性要求：\n"
    "- 只输出纪要正文，不要任何前后说明、致歉或对工具与运行环境的评论。\n"
    "- 通篇凝练、信息密度高；除关键表态外不逐句转述发言，不堆砌直接引语。\n"
    "- 能从逐字稿内容确定会议日期时，把「明天」「下周二」这类相对时间换算成"
    "绝对日期（换算没把握就保留原词）。\n"

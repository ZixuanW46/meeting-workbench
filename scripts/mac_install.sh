...
if [[ "$DRY_RUN" == "1" ]]; then
    ...
    echo "DRY RUN：检查 Homebrew；缺失时请访问 https://brew.sh/"
    ...
    run "$REPO_ROOT/.venv/bin/python" -m pip install -e "$REPO_ROOT[mac]"
    ...
    run npm --prefix "$REPO_ROOT/apps/web" install
    run npm --prefix "$REPO_ROOT/apps/web" run build
...
if ! command -v brew >/dev/null 2>&1; then
    echo "未找到 Homebrew。请访问官方安装页 https://brew.sh/ 安装后重试。" >&2
    exit 1
fi
